...
export const rankSolutions = (solutions: Solution[]): Solution[] => {
  ...
  // Score approved solutions based on weight (cost) and efficiency
  approved.forEach((s) => {
    // Lower weight is better, higher efficiency is better
    const weightScore = 1 / s.profile.weight;
    const efficiencyScore = s.globalEfficiency / 100;
  ...
  rejected.sort(
    (a, b) =>
      Math.max(a.elu.usageIndex, a.els.ratio) -
      Math.max(b.elu.usageIndex, b.els.ratio),
  );
