...
export function SidebarTab({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: ReactNode, label: string }) {
  return (
    <button
      onClick={onClick}
...
export function InputGroup({ label, unit, value, onChange, min, max, step = 0.01 }: { 
...
export function MetricBox({ label, value, unit }: { label: string, value: string, unit: string }) {
  return (
