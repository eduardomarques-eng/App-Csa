...
export class RankingEngine {
  static rankSolutions(solutions: Solution[]): Solution[] {
    ...
    const rejected = solutions.filter(s => !s.isApproved).map(s => ({ ...s, rank: "reprovada" as const }));

    if (approved.length === 0) return rejected;

    // Sort by weight (proxy for cost)
    const sorted = [...approved].sort((a, b) => a.profile.weight - b.profile.weight);
