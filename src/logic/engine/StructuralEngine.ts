...
export class StructuralEngine {
  static calculateSolution(
    ...
    const { mCoef, dCoef } = this.getCoefficients(
      inputs.supportBottom,
      inputs.supportTop,
      inputs.category === "guarda-corpo",
    );
    ...
    const deflection =
      (totalLoad * Math.pow(height, 4) * dCoef * 100000000) /
      (modulusOfElasticity * 1000000 * profile.ix); // mm
    const deflectionLimit = Math.min(
      heightMM / typology.defaultSlsRatio,
      typology.maxDeflectionLimit,
    );
    ...
    const glassStress =
      (glassAlpha *
        windPressure *
        1000 *
        Math.pow(Math.min(height, width), 2)) /
      Math.pow(glassThickness / 1000, 2) /
      1000000;
    ...
    return {
      ...
      ixReq:
        (totalLoad * Math.pow(height, 4) * dCoef * 100000000) /
        (modulusOfElasticity * 1000000 * deflectionLimit),
  ...
  private static getCoefficients(
    start: SupportCondition,
    end: SupportCondition,
    isRailing: boolean,
  ) {
    let mCoef = 0.125;
    let dCoef = 5 / 384;
    ...
    if (isRailing) {
      ...
      if (start === "fixed" && end === "fixed") {
        ...
        (start === "fixed" && end === "pinned") ||
        (start === "pinned" && end === "fixed")
      ) {
        mCoef = 1 / 8;
        dCoef = 1 / 185;
